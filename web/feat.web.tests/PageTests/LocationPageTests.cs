...
public class LocationPageTests
{
	private static LocationModel CreateModel(ISession session)
	{
		...
		var model = new LocationModel(NullLogger<LocationModel>.Instance)
		{
	...
	[Fact]
	public void OnPost_Returns_Page_When_Location_Is_Set_But_No_Distance()
	{
		var session = new TestSession();
		var model = CreateModel(session);
	...
	[Fact]
	public void OnPost_Returns_Page_When_Distance_IsSet_But_No_Location()
	{
		var session = new TestSession();
		var model = CreateModel(session);
	...
	[Fact]
	public void OnPost_Redirects_To_InterestsPage_When_Not_VisitedCheckAnswers()
	{
	...
	[Fact]
	public void OnPost_Redirects_To_CheckAnswers_When_VisitedCheckAnswers_And_Distance_IsNot_ThirtyPlus()
	{
	...
	[Fact]
	public void OnPost_Redirects_To_InterestsPage_When_VisitedCheckedAnswers_And_Distance_ThirtyPlus_And_No_Interests()
	{
	...
	[Fact]
	public void OnPost_Redirects_To_CheckAnswers_When_VisitedCheckedAnswers_And_Distance15_And_Has_Interests()
	{
	...
	[Fact]
	public void OnPost_Redirects_To_CheckAnswers_When_VisitedCheckedAnswers_And_Distance_Is_Null_And_Has_Interests()
	{
		var search = new Search { VisitedCheckAnswers = true, Interests = new List<string> { "Maths", "Riemann zeta function" } };
		var session = new TestSession();
		session.Set("Search", Encoding.UTF8.GetBytes(System.Text.Json.JsonSerializer.Serialize(search)));
